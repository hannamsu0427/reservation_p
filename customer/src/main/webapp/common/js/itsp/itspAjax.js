...
itsp.ajax.CODE_DUP_USER_EMAIL = '89';
...
itsp.ajax.checkDupId = function(obj) {
	...
	itsp.ajax.doPostJSON('/checkDupId', { ID : checkId },
		function(data) {
			itsp.ajax.hideLoading();
...
itsp.ajax.checkDupEmailStep = function(obj) {
	obj = $(obj);
	var checkEmail = obj.val().trim();
	if (itsp.com.isEmptyString(checkEmail)) {
		alert('이메일를 입력해주세요.');
		obj.focus();
		return;
	}
	...
	itsp.ajax.showLoading();
	itsp.ajax.doPostJSON('/checkDupEmailStep', { EMAIL : checkEmail },
		function(data) {
			itsp.ajax.hideLoading();
			if (data.header.code == itsp.ajax.CODE_DUP_USER_EMAIL) {
				alert(checkEmail + '은(는) 이미 등록되어 있어 사용할 수 없습니다.');
			}
			else {
				alert(checkEmail + '은(는) 사용할 수 있습니다.');
				obj.attr('data-itsp-dup-checked', checkEmail);
			}
		}
	);
};

itsp.ajax.checkDupEmail = function(obj) {
	obj = $(obj);
	var checkEmail = obj.val().trim();
	if (itsp.com.isEmptyString(checkEmail)) {
		alert('이메일을 입력해주세요.');
	...
	itsp.ajax.showLoading();
	itsp.ajax.doPostJSON('/teacher/checkDupEmail', { EMAIL : checkEmail },
		function(data) {
			itsp.ajax.hideLoading();
			if (data.header.code == itsp.ajax.CODE_DUP_USER_ID) {
				alert(checkEmail + '은(는) 이미 등록되어 있어 사용할 수 없습니다.');
